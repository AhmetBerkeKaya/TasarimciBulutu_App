...
import '../../data/models/showcase_post_model.dart'; // Yeni modelimizi import edelim
...
class ApiService {
  ...
  // --- Showcase (YENİ) ---
  Future<List<ShowcasePost>> getAllShowcasePosts({int page = 0, int limit = 20}) async {
    try {
      final int skip = page * limit;
      final response = await _dio.get('/showcase/', queryParameters: {'skip': skip, 'limit': limit});
      return (response.data as List).map((json) => ShowcasePost.fromJson(json)).toList();
    } on DioException catch (e) {
      print('getAllShowcasePosts DioException: ${e.response?.data}');
      return [];
    }
  }

  Future<ShowcasePost> createShowcasePost({required String title, String? description, required String filePath}) async {
    try {
      ...
      final response = await _dio.post('/showcase/', data: formData);
      return ShowcasePost.fromJson(response.data);
    } on DioException catch (e) {
      print('createShowcasePost DioException: ${e.response?.data}');
      throw Exception('Vitrin gönderisi oluşturulamadı.');
    }
  }
  ...
  Future<Map<String, dynamic>> getViewerToken() async {
    try {
      final response = await _dio.get('/aps/viewer-token');
      // Dio, gelen JSON yanıtını otomatik olarak Map'e çevirir.
