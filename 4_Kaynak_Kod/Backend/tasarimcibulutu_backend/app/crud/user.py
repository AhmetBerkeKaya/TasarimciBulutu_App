...
def update_user(db: Session, user_id: UUID, user_update: schemas.UserUpdate) -> Optional[models.User]:
    ...
    # Değişen alanları ve eski/yeni değerlerini tespit et
    ...
    # Eğer en az bir alanda değişiklik varsa, denetim kaydı oluştur
    if changes:
...
def delete_user(db: Session, user_id: str) -> models.User | None:
    ...
    if not db_user:
        return None
    # Kullanıcı silinmeden önce bir denetim kaydı oluşturalım
    audit_crud.create_audit_log(
        db, 
        user_id=db_user.id, 
        action="USER_DELETED",
        # Silme işleminde actor_id'yi belirtmek önemlidir (eğer bir admin siliyorsa)
        # Şimdilik kullanıcının kendi kendini sildiğini varsayıyoruz.
    )
    db.delete(db_user)
    db.commit()
    return db_user
...
def update_user_password(db: Session, user: models.User, new_password: str):
    # Denetim kaydı oluştur (şifrenin kendisini değil, olayı kaydet)
    audit_crud.create_audit_log(
        db,
        user_id=user.id,
